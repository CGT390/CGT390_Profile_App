Lab 14 Reflection:
This lab I created useFilteredCards to make a custom hook to handle the filtering of cards. Sometimes the filtering is used in multiple places, so making this cusotm hook is helpful for when I need to handle custom hooks more often. I also created useProfileSubmit for submitting a profile to my list of data. Each of these custom hooks essentially makes my code look much more simple, and allows all of the seemingly complicated/more complex state varibles to be handled in a different place. This is very important, because when apps scale up to larger proportions, this makes the code a lot more reusable. This will make my code a lot faster in the long run.  
